...
import { ArrowLeft, Inbox, Loader2, MessageSquarePlus, Phone, Sparkles } from 'lucide-react'
import { getMySiteFeedback, type SiteFeedbackRow } from '@/services/siteFeedback.service'
          ...
          <FadeIn className="space-y-3" y={14}>
            {rows.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className="rounded-2xl border border-line bg-surface p-4 transition-all duration-300 ease-apple hover:-translate-y-0.5 hover:shadow-card-hover sm:p-5"
                >
                    ...
                    <div className="flex min-w-0 items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        <div className="text-[13.5px] font-semibold text-text">
                          {t(`site_feedback.kind.${r.kind}.label`)}
                        </div>
                        <div className="text-[11.5px] text-text-muted">
                          {fmtDate(r.created_at)}
                          {r.page_label ? ` · ${r.page_label}` : ''}
                        </div>
                      </div>
                    </div>
