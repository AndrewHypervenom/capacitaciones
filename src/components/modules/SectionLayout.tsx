...
export function SectionLayout({ style = 'default', hasMedia, children, className }: Props) {
  ...
  if (style === 'side-by-side' && hasMedia) {
    return (
      <div className={cn('grid md:grid-cols-2 gap-10 items-start', className)}>
        {children}
      </div>
    );
  }
